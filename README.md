...
一个无需后端的本地静态页面，用于登记、检索和统计病人问诊记录。页面数据保存在浏览器 localStorage 中，双击打开 `index.html` 即可使用。
...
3. 打包完成后，在 `release` 目录下会生成一个 `DoctorRegister-1.0.0-portable.exe`
...
3. 成品同样会输出到 `release/DoctorRegister-1.0.0-portable.exe`
...
- `DoctorRegister-1.0.0-portable.exe` 是单文件便携版，首次运行时部分电脑可能会被系统安全策略、杀毒软件或临时目录权限拦截
...
3. 该脚本会依次运行网页测试并重新打包 Windows portable 版，最后校验 `release/DoctorRegister-1.0.0-portable.exe` 是否真实生成

### GitHub Actions

- 工作流文件位于 `.github/workflows/ci.yml`
- `ubuntu-latest` 负责执行网页测试
- `windows-latest` 负责执行桌面版打包校验，并上传 `DoctorRegister-1.0.0-portable.exe` 作为构建产物
...
- 使用的存储键为 `doctor.consultation.records.v1`。
- 不依赖服务器，不会自动同步到其他浏览器或其他电脑。
